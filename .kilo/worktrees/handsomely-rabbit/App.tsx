...
export default function App() {
    ...
    const [appLoading, setAppLoading] = useState({
        isActive: true, // Start true for boot screen
        progress: 0,
        text: 'Démarrage de BERAMETHODE',
    ...
    useEffect(() => {
        ...
        const bootApp = async () => {
            ...
            setAppLoading({ isActive: true, progress: 10, text: 'BERAMETHODE V2', subText: 'Connexion à la base...' });
            await new Promise(r => setTimeout(r, 200));
            setAppLoading(prev => ({ ...prev, progress: 35, subText: 'Chargement des paramètres...' }));
            await new Promise(r => setTimeout(r, 300));
            
            // Wait for Auth to settle before claiming 100%
            if (!authLoading) {
                setAppLoading(prev => ({ ...prev, progress: 85, subText: 'Préparation du tableau de bord...' }));
                await new Promise(r => setTimeout(r, 400));
                setAppLoading(prev => ({ ...prev, progress: 100, subText: 'Prêt.' }));
                await new Promise(r => setTimeout(r, 250)); // let the user see 100%
    ...
    // Helper to wrap heavy actions in a loader manually
    const withLoading = async (text: string, action: () => void | Promise<void>) => {
        setAppLoading({ isActive: true, progress: 10, text, subText: 'Veuillez patienter...' });
        await new Promise(r => setTimeout(r, 100)); // small yield for UI to show loader
        setAppLoading(prev => ({ ...prev, progress: 50 }));
        
        // Execute the heavy block
        await action();
        
        setAppLoading(prev => ({ ...prev, progress: 100, subText: 'Terminé' }));
        await new Promise(r => setTimeout(r, 150)); // let user see 100
        setAppLoading(prev => ({ ...prev, isActive: false }));
    };
